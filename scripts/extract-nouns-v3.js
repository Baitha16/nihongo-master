...
// Parse vocabulary - FIXED to get full meaning
const words = [];
let i = 0;
...
while (i < lines.length) {
  ...
  if (isNumber(line)) {
    ...
    while (j < lines.length) {
      ...
      // If it's not empty and not Japanese, it's part of the meaning
    ...
    if (japanese && meaningParts.length > 0 && japanese.length > 0) {
      // Join all meaning parts and clean up
...
words.slice(0, 20).forEach(w => console.log(`  ${w.kanji} | ${w.japanese} | ${w.meaning}`));

// Check specific words
console.log('\nChecking specific words:');
const checkWords = ['あのひと', 'でんわ', 'でんき', 'ともだち', 'みなさん'];
checkWords.forEach(word => {
  const found = words.find(w => w.japanese === word);
  if (found) {
    console.log(`  ✓ ${found.japanese}: ${found.meaning}`);
  } else {

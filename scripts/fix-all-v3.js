...
// Check if a string is Japanese (hiragana, katakana, or kanji)
function isJapaneseChar(str) {
...
function isJapanese(str) {
  ...
  // Allow Japanese characters, spaces, and common Japanese punctuation
...
function processVerbs() {
  ...
  while (i < lines.length) {
    ...
    if (isNumber(line)) {
      ...
      // Collect all parts until next number
      while (j < lines.length) {
        ...
        // If it's a number but not the entry number, it's part of meaning (like "1" in "Tiba 1")
        if (/^\d+$/.test(nextLine) && meaningParts.length > 0) {
...
console.log('\n--- Sample Verbs ---');
verbQuestions.slice(0, 10).forEach(q => {
  console.log(`${q.reading} (${q.romaji}): ${q.meaning}`);
});

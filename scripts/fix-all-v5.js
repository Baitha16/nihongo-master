...
function hasKanji(str) {
  return /[\u4E00-\u9FFF\uF900-\uFAFF]/.test(str);
}

// Check if string is hiragana only
function isHiragana(str) {
  return /^[\u3040-\u309F]+$/.test(str);
}

// Check if string is katakana only
  ...
  while (i < lines.length) {
    ...
    if (isNumber(line)) {
      ...
      while (j < lines.length) {
        ...
        if (isJapanese(nextLine)) {
          if (phase === 'meaning') {
            phase = 'japanese';
          }
          
          if (phase === 'japanese') {
            // Check if this line has kanji
            if (hasKanji(nextLine)) {
              // This is the kanji line
              kanjiLine = nextLine;
              phase = 'done';
            } else {
              // This is part of the hiragana reading
              japaneseLines.push(nextLine);
            }
          } else if (phase === 'done') {
            // We already have kanji, skip extra Japanese lines
          }
          j++;
          continue;
        }
      ...
      let japanese = japaneseLines.join('');
      ...
      // Get kanji
      let kanji = kanjiLine || japanese;

      if (japanese.length > 0 && meaning.length > 0) {
        // Check for duplicates
        const existingIndex = verbs.findIndex(v => v.japanese === japanese && v.meaning === meaning);
        if (existingIndex === -1) {
          verbs.push({
            japanese: japanese,
            kanji: kanji,
...
checkWords.forEach(check => {
  ...
  if (found && found.length > 0) {
    found.forEach(q => {
      console.log(`✓ ${q.reading} (${q.romaji}): ${q.meaning} [Kanji: ${q.kanji}]`);
    });
  } else {
...
// Sample from verbs
console.log('\n--- Sample Verbs (first 20) ---');
verbQuestions.slice(0, 20).forEach(q => {
  console.log(`${q.reading} (${q.romaji}): ${q.meaning} [${q.kanji}]`);
});
